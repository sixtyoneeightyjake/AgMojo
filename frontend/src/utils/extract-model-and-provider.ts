...
export const extractModelAndProvider = (model: string) => {
  ...
  if (split.length === 1) {
    ...
    // Treat known Gemini models (from VERIFIED_MODELS) as gemini provider when no prefix
    if (["gemini-2.5-pro", "gemini-2.5-flash"].includes(split[0])) {
      return { provider: "gemini", model: split[0], separator: "/" };
    }
