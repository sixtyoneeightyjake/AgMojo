...
export function ChatInterface() {
  ...
  const handleSendMessage = async (
    ...
    if (chatMode === "brainstorm") {
      // Brainstorm path: no file/image handling, simple LLM completion
      setBrainstormMessages((prev) => [...prev, { role: "user", content }]);
      try {
        const allMessages = [
          ...brainstormMessages.map((m) => ({ role: m.role, content: m.content })),
          { role: "user" as const, content },
        ];
        const { content: reply } = await OpenHands.brainstorm(allMessages);
  ...
  return (
    <ScrollProvider value={scrollProviderValue}>
      <div className="h-full flex flex-col justify-between">
        ...
        >
          {chatMode === "agent" && isLoadingMessages && (
        ...
        <div className="flex flex-col gap-[6px] px-4 pb-4">
          <div className="flex justify-between relative">
            ...
            <div className="absolute left-1/2 transform -translate-x-1/2 bottom-0">
              {chatMode === "agent" && curAgentState === AgentState.RUNNING && (
                <TypingIndicator />
              )}
            </div>
          ...
          <InteractiveChatBox
            ...
            onStop={chatMode === "agent" ? handleStop : () => {}}
            isDisabled={
              chatMode === "agent"
