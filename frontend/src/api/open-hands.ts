...
class OpenHands {
  ...
  /**
   * Prewarm the backend runtime (build image / warm server) in background
   */
  static async prewarmRuntime(): Promise<{ status: string; runtime: string } | null> {
    try {
      const { data } = await openHands.post<{ status: string; runtime: string }>(
        "/api/runtime/prewarm",
      );
      return data;
    } catch (e) {
      // Ignore errors here; prewarm is best-effort
      return null;
    }
  }

  /**
   * Brainstorm completion (no code generation). Uses a dedicated system prompt and Gemini Flash.
   */
  static async brainstorm(
    messages: { role: "system" | "user" | "assistant"; content: string }[],
  ): Promise<{ content: string }> {
    const { data } = await openHands.post<{ content: string }>(
      "/api/brainstorm",
      { messages },
    );
    return data;
