...
async def create_new_conversation(
    ...
    # Generate title immediately if we have an initial user message
    if initial_user_msg and initial_user_msg.strip():
        try:
            # Load user settings to get LLM config
            settings_store = await SettingsStoreImpl.get_instance(config, user_id)
            settings = await settings_store.load()

            if settings and settings.llm_model:
                # Create OpenHandsConfig with LLM settings
                llm_config = setup_llm_config(config, settings)

                # Create LLM registry with the proper config
                llm_registry = LLMRegistry(llm_config)
                llm_title = await generate_conversation_title(
                    initial_user_msg, llm_config.get_llm_config(), llm_registry
                )

                if llm_title:
                    # Update conversation metadata with the new title
                    conversation_metadata.title = llm_title
                    conversation_store = await ConversationStoreImpl.get_instance(config, user_id)
                    await conversation_store.save_metadata(conversation_metadata)
                    logger.info(f'Generated title at creation time: {llm_title}')

        except Exception as e:
            # Don't fail the conversation creation if title generation fails
            logger.warning(f'Failed to generate title at creation time: {e}')
