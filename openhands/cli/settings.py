...
async def modify_llm_settings_basic(
    ...
    try:
        ...
        if provider == 'gemini':
            provider_models = [
                m for m in provider_models if m not in VERIFIED_GEMINI_MODELS
            ]
            provider_models = VERIFIED_GEMINI_MODELS + provider_models
        ...
        elif provider == 'gemini' and VERIFIED_GEMINI_MODELS:
            # Use the first model in the VERIFIED_GEMINI_MODELS list
            default_model = VERIFIED_GEMINI_MODELS[0]
