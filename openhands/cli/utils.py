...
def extract_model_and_provider(model: str) -> ModelInfo:
    ...
    if len(split) == 1:
        ...
        if split[0] in VERIFIED_GEMINI_MODELS:
            return ModelInfo(provider='gemini', model=split[0], separator='/')
...
VERIFIED_PROVIDERS = ['openai', 'gemini']
...
VERIFIED_OPENHANDS_MODELS: list[str] = []

# New: verified Gemini models for provider inference when no prefix is present
